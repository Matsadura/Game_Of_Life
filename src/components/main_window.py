...
class GameOfLifeMainWindow:
    def __init__(self):
        ...
        self.grid_canvas.draw_grid_2()  # Draw the initial grid
    ...
    def run_game(self):
        if hasattr(self, 'is_running') and self.is_running:  # Check if is_running is defined
            new_corr = self.game.update()
            # print("new:", new_corr)
            self.grid_canvas.draw_grid(new_corr, self.game.grid)
    ...
    def cell_click(self, event):
        ...
        if 0 <= row < self.settings["rows"] and 0 <= col < self.settings["cols"]:
            ...
            # self.grid_canvas.canvas.itemconfig(self.grid_canvas.cells[row][col], fill="black" if self.game.grid[row][col] == 1 else "white")
            self.grid_canvas.draw_grid_2()  # Redraw the grid after toggling
