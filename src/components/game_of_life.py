    ...
    def update(self):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                if self.grid[r][c] == 1 and alive_neighbors not in (2, 3):  # Cell is alive
                    # new_grid[r][c] = 1 if alive_neighbors in (2, 3) else 0
                    new_grid.append((r, c, 0))
                elif self.grid[r][c] == 0 and alive_neighbors == 3:  # Cell is dead
                    # new_grid[r][c] = 1 if alive_neighbors == 3 else 0
                    new_grid.append((r, c, 1))
